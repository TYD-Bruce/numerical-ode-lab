...
describe("production expression migration boundary", () => {
  ...
  const odeApp = source("../../frontend/src/labs/ode/odeApp.ts");
  const solvers = source("../../packages/numerics/src/ode/solvers.ts");
  const productionExpression = source(
    "../../frontend/src/labs/ode/problemExpressions.ts",
  );
  const convergenceView = source(
    "../../frontend/src/labs/ode/convergenceStudyView.ts",
  );

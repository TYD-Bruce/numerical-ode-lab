...
function createComposedCards(): readonly ResolvedGlossaryEntry[] {
  const registry = createGlossaryRegistry({
    coreEntries: [...coreGlossaryEntries, ...odeGlossaryEntries],
    extensions: [odeGlossaryExtension],
    policy: strict,
  });
  return teachingOrder.map((id) => {
    const termId = defineGlossaryTermId(id, strict)!;
    return registry.resolveById("ode", termId)!;
  });
}

function liveIds(entry: ResolvedGlossaryEntry): readonly string[] {
  const relationIds = (
    fields: readonly GlossaryRelatedTerm[] | undefined
  ): string[] =>
    (fields ?? [])
      .filter(
        (relation): relation is Extract<
          GlossaryRelatedTerm,
          { kind: "term" }
        > => relation.kind === "term"
      )
  ...
  return [
    ...
    ...relationIds(entry.relatedTerms),
    ...relationIds(entry.commonlyConfusedTerms),
  ];
}

describe("ODE Glossary Wave 1 inert content", () => {
  it("owns exactly eight complete ODE entries with exact approved content", () => {
    expect(odeGlossaryEntries).toHaveLength(8);
    expect(odeGlossaryEntries).toEqual(expectedOdeEntries);
  });

  it("owns exactly two context-only Core overrides", () => {
    expect(odeGlossaryExtension).toEqual(expectedOdeExtension);
    expect(odeGlossaryExtension.overrides).toHaveLength(2);
    expect(odeGlossaryExtension.overrides.map(({ termId }) => termId)).toEqual([
      "numerical_approximation",
      "explicit_scheme",
    ]);

    for (const override of odeGlossaryExtension.overrides) {
      expect(Object.keys(override).sort()).toEqual(
        Object.keys(
          expectedOdeExtension.overrides.find(
            ({ termId }) => termId === override.termId
          )!
        ).sort()
      );
      expect(override).not.toHaveProperty("id");
      expect(override).not.toHaveProperty("label");
      expect(override).not.toHaveProperty("aliases");
      expect(override).not.toHaveProperty("definition");
      expect(override).not.toHaveProperty("fullDefinition");
      expect(override).not.toHaveProperty("intuition");
      expect(override).not.toHaveProperty("assumptionsAndLimits");
      expect(override).not.toHaveProperty("misconception");
    }
  });

  it("composes exactly ten cards in the approved teaching order", () => {
    const cards = createComposedCards();
    expect(cards).toHaveLength(10);
    expect(cards.map(({ id }) => id)).toEqual(teachingOrder);
    expect(new Set(cards.map(({ id }) => id)).size).toBe(10);
    expect(cards.some(({ id }) => id === "implicit_scheme")).toBe(false);
  });

  it("applies exact ODE context without rewriting canonical Core fields", () => {
    const coreBefore = JSON.stringify(coreGlossaryEntries);
    const cards = createComposedCards();
    const numerical = cards.find(
      ({ id }) => id === "numerical_approximation"
    )!;
    const explicit = cards.find(({ id }) => id === "explicit_scheme")!;

    expect(numerical).toMatchObject({
      ...coreGlossaryEntries[0],
      moduleId: "ode",
      display: "Numerical approximation",
      whyItMattersHere:
        expectedOdeExtension.overrides[0].whyItMattersHere,
      prerequisiteTermIds: ["initial_value_problem", "time_grid"],
      relatedTerms: expectedOdeExtension.overrides[0].relatedTerms,
      commonlyConfusedTerms:
        expectedOdeExtension.overrides[0].commonlyConfusedTerms,
      moduleNote: expectedOdeExtension.overrides[0].moduleNote,
      tutorTopic:
        "Explain what a displayed u_n represents and distinguish it from exact value, error, and residual.",
    });
    expect(explicit).toMatchObject({
      ...coreGlossaryEntries[1],
      moduleId: "ode",
      display: "Explicit scheme",
      whyItMattersHere:
        expectedOdeExtension.overrides[1].whyItMattersHere,
      formula: expectedOdeExtension.overrides[1].formula,
      prerequisiteTermIds: ["numerical_approximation", "step_size"],
      relatedTerms: expectedOdeExtension.overrides[1].relatedTerms,
      commonlyConfusedTerms:
        expectedOdeExtension.overrides[1].commonlyConfusedTerms,
      moduleNote: expectedOdeExtension.overrides[1].moduleNote,
      tutorTopic:
        "Identify which current quantities are known before the selected explicit update is evaluated.",
    });
    expect(JSON.stringify(coreGlossaryEntries)).toBe(coreBefore);
  });

  it("preserves every exact normalized formula/accessibility pair", () => {
    const cards = createComposedCards();
    expect(
      Object.fromEntries(cards.map(({ id, formula }) => [id, formula]))
    ).toEqual({
      ordinary_differential_equation: {
        latex: "y'(t)=f(t,y(t))",
        accessibleText: "y prime of t equals f evaluated at t and y of t.",
  ...
  it("resolves every live relationship and retains future labels as inert text", () => {
    const cards = createComposedCards();
    ...
    for (const card of cards) {
      const ids = liveIds(card);
      expect(ids).not.toContain(card.id);
      for (const id of ids) expect(registered.has(id)).toBe(true);
      expect(new Set(card.prerequisiteTermIds ?? []).size).toBe(
        card.prerequisiteTermIds?.length ?? 0
      );
      for (const relations of [
        card.relatedTerms,
        card.commonlyConfusedTerms,
      ]) {
        const keys = (relations ?? []).map((relation) =>
          relation.kind === "term"
            ? `term:${relation.termId}`
    ...
    expect(serialized).not.toContain('"termId":"implicit_scheme"');
    expect(serialized).toContain('"kind":"future","label":"implicit scheme"');
    expect(
      cards.flatMap((card) => [
        ...(card.relatedTerms ?? []),
        ...(card.commonlyConfusedTerms ?? []),
      ])
    ).toContainEqual({ kind: "future", label: "A-stability" });
  });

  it("exports and composes deeply immutable data", () => {
    expectDeeplyFrozen(odeGlossaryEntries);
    expectDeeplyFrozen(odeGlossaryExtension);
    const cards = createComposedCards();
    for (const card of cards) expectDeeplyFrozen(card);

    expect(() => {
      (
        odeGlossaryEntries as unknown as Array<{
          label: string;
        }>
      )[0]!.label = "Changed";
    }).toThrow(TypeError);
    expect(() => {
      (
        odeGlossaryExtension.overrides[0]!.relatedTerms as unknown as Array<{
          kind: "future";
          label: string;
        }>
      )[1]!.label = "Changed";
  ...
  it("contains plain content only and no activation or evidence metadata", () => {
    const data = {
      core: coreGlossaryEntries,
      ode: odeGlossaryEntries,
      extension: odeGlossaryExtension,
    };
    ...
    expect(serialized).not.toContain("references/private");
    expect(serialized).not.toContain("getGlossaryBinding");
    expect(serialized).not.toContain("GlossaryHost");
    expect(serialized).not.toContain("TutorInjector");
    expect(serialized).not.toContain("ODE-W1-ANN-");
  });
});
